...
namespace th
{
	...
	bool TH10Reader::getItems(std::vector<Item>& items) noexcept
	{
		...
		for (int_t i = 0; i < /*2000*/100; ++i)
		{
			...
			if (eax == 1)
			{
				...
				readMemoryBuffer(ebp - 0x4, buffer, sizeof(buffer));
				item.x = readBuffer<float32_t>(buffer + 0x4 - 0x4);
				item.y = readBuffer<float32_t>(buffer + 0x4);
				item.dx = readBuffer<float32_t>(buffer + 0x4 + 0x8);
				item.dy = readBuffer<float32_t>(buffer + 0x4 + 0xC);
				item.height = item.width = 6.0f;
				item.type = readBuffer<int32_t>(buffer + 0x4 + 0x30);
				// 正常点分为以下几种
		...
		//std::cout << "getItems " << maxI << std::endl;
		return true;
	}
	...
	bool TH10Reader::getBullets(std::vector<Bullet>& bullets) noexcept
	{
			...
			if (bp)
			{
				...
				if (eax)
				{
					...
					if (!(eax & 0x00000400))
					{
						...
						readMemoryBuffer(ebx + 0x3B4, buffer, sizeof(buffer));
						bullet.x = readBuffer<float32_t>(buffer - 0x3B4 + 0x3B4);
						bullet.y = readBuffer<float32_t>(buffer - 0x3B4 + 0x3B8);
						bullet.dx = readBuffer<float32_t>(buffer - 0x3B4 + 0x3C0);
						bullet.dy = readBuffer<float32_t>(buffer - 0x3B4 + 0x3C4);
						bullet.width = readBuffer<float32_t>(buffer - 0x3B4 + 0x3F0);
						bullet.height = readBuffer<float32_t>(buffer - 0x3B4 + 0x3F4);
		...
		//std::cout << "getBullets " << maxI << std::endl;
		return true;
	}
