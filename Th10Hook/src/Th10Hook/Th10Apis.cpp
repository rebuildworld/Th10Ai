...
{
	template <typename T>
	inline T ReadMemory(uint_t address)
	{
	...
	bool Th10Apis::GetItemsTo(ManagedVector<Item>& items)
	{
		items.clear();
	...
	bool Th10Apis::GetEnemiesTo(ManagedVector<Enemy>& enemies)
	{
		enemies.clear();
	...
	bool Th10Apis::GetLasersTo(ManagedVector<Laser>& lasers)
	{
		lasers.clear();
