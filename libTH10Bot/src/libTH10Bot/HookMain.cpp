...
#define HOOK_DESTROY (WM_USER + 0x1235)
	...
	void WINAPI HookMain()
	{
		try
		{
			std::string logName = Utils::GetModuleDir(g_module) + "/TH10Bot_%N.log";
			bl::add_file_log
			(
			...
			if (!g_created)
				THROW_BASE_EXCEPTION(Exception() << err_str(u8"TH10Bot创建失败，详细信息请查看libTH10Bot.log。"));
			ON_SCOPE_EXIT([]()
			{
				if (g_destroy)
				{
	...
	void HookCreate()
	{
		try
		{
			std::string logName = Utils::GetModuleDir(g_module) + "/libTH10Bot_%N.log";
			bl::add_file_log
			(
				bl::keywords::file_name = logName,
				bl::keywords::rotation_size = 10 * 1024 * 1024,
